...
setup(
    ...
    install_requires=["pybase64","python-rapidjson"], # libraries qui seront automatiquement installées lorsque nous installerons notre librairie depuis l’extérieur.  contrairement à ce qu’il y avait dans le fichier requirements.txt,  on ne fixe pas précisement les versions des dépendances, pour éviter un éventuel conflit de versions si une application qui utilisait la librairie dépendait elle-même d’une version précise d'un librairie. Si la librairie n'est pas compatible avec n’importe quelle version des dépendances, restreindre les versions compatibles, par exemple : requests>=2.0.0,<3.
    extras_require={        
            "dev": ["apply","numpy"],   #dépendances nécessaires lorsque l’on développe la librairie, notamment les dépendances utilisées dans les tests unitaires. Mais elles ne sont pas nécessaires pour le bon usage de la librairie dans le cas nominal.
      },       
