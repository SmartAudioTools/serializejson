...
def test_serialize_vs_pickle():
    ...
    serializers = {
        ...
        "pickle_protocol_4": {"encoder": lambda obj: pickle.dumps(obj, protocol=4), "decoder": pickle.loads},
        ...
        "serializejson": {
            "encoder": serializejson.Encoder(
                attributes_filter=False, numpy_types_to_python_types=False, indent=None, numpy_array_readable_max_size=4
            ),
        ...
        "serializejson_bytes": {
            "encoder": serializejson.Encoder(
                attributes_filter=False, numpy_types_to_python_types=False, indent=None, return_bytes=True
            ),
            "decoder": serializejson.Decoder(),
        },
                    ...
                    if categoryName.startswith("object"):
                        ...
                        if not same_as_original:
                            ...
                            if repr_value != repr_loaded:
                                message = "   %s \n->%s \n-> %s\n" % (repr_value, repr(dumped[:200]), repr_loaded)
                            else:
                                message = "   %s (%s)\n->%s \n-> %s (%s)\n" % (
                                    repr_value,
                            ...
                            if serializerName.startswith("serializejson_numpy_array_to_list") and (
                                ...
                                if type_value in (
                                    ...
                                    raise_exception = False
                            elif serializerName in ("jsonpickle", "serpent"):  # and (value is type(None)):
                                raise_exception = False
                            if raise_exception:
                                raise Exception(message)
        if all_ok:
