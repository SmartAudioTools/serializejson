...
def tuple_from_ndarray(inst, classe):
    ...
    if (
        ...
        or (instCont.dtype == "int32" and max(instCont.min(), instCont.max(), key=abs) <= 9999)
        or (instCont.dtype == "int16" and max(instCont.min(), instCont.max(), key=abs) <= 9)
    ):
        ...
        if serializeParameters.use_numpyB64_bytesB64_bytearrayB64:
            if instCont.dtype == bool:
                return (
                    numpyB64,
                    (encodedB64(numpy.packbits(instCont.astype(numpy.uint8))), "bool", len(instCont)),
                    None,
                )
            else:
        ...
        if serializeParameters.use_numpyB64_bytesB64_bytearrayB64:
            if instCont.dtype == bool:
                return (
                    numpyB64,
                    (encodedB64(numpy.packbits(instCont.astype(numpy.uint8))), "bool", instCont.shape),
                    None,
                )
            else:
