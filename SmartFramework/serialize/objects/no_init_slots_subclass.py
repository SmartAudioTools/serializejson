...
class C_New_SaveDict_RestoreNothing_subclass(BaseClass):  # ne sert pas à grand chose , sauf si on veut se garder la posibilitée de restaurer l'state plus tard
    __slots__ = ('par3',)
    def __init__(self, par1='defaut1', par2='defaut2',par3 = 'default3'):
        log('        __init__(' + par1 + ',' + par2 + ',' + par3 + ')')
        self._par1 = par1
...
class C_New_SaveDict_SetState_subclass(BaseClass):  # sert a pouvoir executer code spécifique a la restauration
    ...
    def __setstate__(self, state):
        if isinstance(state,tuple): # pour compatibilité pickle, qui passe un tuple avec (etat du __dict__ , etat des slots)
            state = state[1]
