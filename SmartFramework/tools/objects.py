...
NoneType = type(None)
builtInTypes = (type(None), bool, memoryview, bytearray, bytes, complex, dict, float, frozenset, int, list, range, set, slice, str, tuple)
...
def deepCompare(a, b, return_reason=False):
    ...
    if hasattr(a, "__slots__"):
        for base_classe in a.__class__.__mro__[:-1]: # on ne prend pas le dernier qui est toujours (?) object
            for slot in getattr(base_classe, '__slots__', ()): # on utilise pas directement base_classe.__slots__  car une classe de base n'a pas forcement redefinit __slots__ 
                if hasattr(a, slot):
                    if hasattr(b, slot):
...
def isPyQt(obj):
    return hasattr(obj, 'disconnect')  # ATTENTION SI CHANGE, DOIT CHANGER EGALEMENT LE HACK (FAUSSE METHODE) DANS SmarFace/models.py  pyqtConfigure ne marche pas avec PySide2


def isInstance(obj):
    return hasattr(obj, '__new__') and not (isinstance(obj, builtInTypes) or inspect.isclass(obj) or inspect.isfunction(obj) or inspect.ismodule(obj))  # permet d'eliminer les types de base (int, float etc et les fonctions)
...
def tuple_from_ndarray(inst, classe):
    ...
    if (not serializeParameters.numpy_array_dumped_base64)or (instCont.size <= serializeParameters.numpy_array_readable_max_size) or (instCont.dtype == 'int32' and max(instCont.min(), instCont.max(), key=abs) <= 9999) or (instCont.dtype == 'int16' and max(instCont.min(), instCont.max(), key=abs) <= 9):
        ...
        if serializeParameters.use_numpyB64_bytesB64_bytearrayB64:
            if instCont.dtype == bool:
                return (numpyB64, (encodedB64(numpy.packbits(instCont.astype(numpy.uint8))), 'bool', len(instCont)), None)
            else:
        ...
        if serializeParameters.use_numpyB64_bytesB64_bytearrayB64:
            if instCont.dtype == bool:
                return (numpyB64, (encodedB64(numpy.packbits(instCont.astype(numpy.uint8))), 'bool', instCont.shape), None)
            else:
                return (numpyB64, (encodedB64(instCont), instContdtype, instCont.shape), None)
        else:
            import numpy
            return (numpy.ndarray, (instCont.shape, instContdtype, bytearray(instCont)), None)
...
def tuple_from_datetime(inst, classe):
    return (classe, (inst.year, inst.month, inst.day, inst.hour, inst.minute, inst.second, inst.microsecond), None)
...
def instance(__class__=object, __init__=None, __state__=None, __initArgs__=None, **argsSup):
    ...
    try:
        ...
        if isinstance(__class__, str):
        ...
        elif isinstance(__class__, dict):                    # permet de gere le cas ou on donne directement un dictionnaire en premier argument
            ...
            if isclass(__class__):
                ...
                raise Exception("erreure lors de la creation d'instance le premier parametre de Instance() n'est ni une classe , ni string representant un classe , ni une instance, ni un dictionnaire, ni un callable (fonction)")
    ...
    if __state__ or argsSup:
        if __state__ and type(__state__) == dict:
            ...
            __state__ = argsSup
        if __state__:
            if hasattr(inst, "__setstate__"):  # j'ai du remplacer hasMethod(inst,"__setstate__") par hasattr(inst,"__setstate__") pour pouvoir deserialiser des sklearn.tree._tree.Tree en json "__setstate__" n'est pas reconnu comme étant une methdoe !? alors que bien là .
                inst.__setstate__(__state__)
            else:
                if type(__state__) is dict:
                    if serializeParameters.set_attributs:  # si la variable global setAttributs = True , il tente de faire appel aux setters (NON TESTE)
                        for key, value in __state__.items():
                            ...
                            if hasattr(inst, attributSetMethode) :
                                ...
                                if hasattr(inst, attributSetMethode) :
                                    methode = eval('inst.' + attributSetMethode)
                                    methode(value)
                                inst.__setattr__(key, value) # permet de gerer à la fois les cas ou key est une propriétée, un attriut dans __dict__ ou dans __slot__
                    else:
                        if hasattr(inst, "__slots__"):
                            for key, value in __state__.items():
